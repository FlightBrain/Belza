...
// Per-request network timeout for Slack calls. Slack normally answers well
// under a second; anything past this is a hung connection, not a slow one.
...
export async function slackApi(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    const res = await fetch(`https://slack.com/api/${method}?${query}`, {
      ...
      // A hung Slack connection would otherwise hold the invocation until
      // Vercel kills it, producing no reply and no error.
