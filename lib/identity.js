...
// includeBots: match apps by name as well as people.
//
// Off by default and ON only for an explicit person lookup. The asymmetry is
// deliberate. "whats the notion pricing page" must NOT match the Notion app as
// someone being discussed - that would inject "an app, not a teammate" context
// into a work question. But "who is Notion" absolutely must resolve, because
// otherwise mentionedContext is empty and the model has no grounding at all
// and is free to invent a colleague named Notion. A tag already got this right;
// a typed name did not.
export function resolveByName(text, roster, { excludeUserId, includeBots = false } = {}) {
  ...
  const candidates = includeBots
    ? (roster?.people || []).filter((p) => !p.deleted)
    : humans(roster);
...
export function resolvePeople({ rawText, roster, botUserId = '', excludeUserId, includeBots = false } = {}) {
  ...
  const byName = resolveByName(substituted, roster, { excludeUserId, includeBots });
