...
const INTENT_PATTERNS = [
  ...
  { intent: 'feedback', patterns: [
    /^\s*feedback[\s:\-]/i,
    // "feedback" as a leading topic word ("some feedback ...", "quick
    // feedback ...") rather than only a bare prefix. Anchored near the start
    // of the message so this doesn't fire on unrelated mid-sentence uses
    // like "does anyone have feedback on the playbook".
    /^\s*(hey[,\s]+)?(some|a\s+bit\s+of|quick|little|just\s+a)\s+feedback\b/i,
    ...
    /\bnot\s+helpful\b/i,
    /\bwrong\s+answer\b/i,
    // Broader corrective phrasing for casual in-thread feedback (e.g. "no
    // that's not right", "you missed the meeting time"). Kept declarative/
    // sentiment-specific rather than matching bare "no" or "wrong" so a real
    // follow-up question in the thread doesn't get swallowed as feedback.
    /\bnot\s+(quite|right|correct)\b/i,
    /\byou\s+missed\b/i,
    /\bthat'?s\s+not\s+(right|correct|it|what\s+i\s+(meant|asked|said))\b/i,
    /\bnope,?\s+that'?s\b/i,
    /\bactually,?\s+no\b/i,
    /\bthat\s+didn'?t\s+work\b/i,
    /\byou\s+misunderstood\b/i,
  ]},
...
// Deliberately narrower than WORK_KEYWORDS — "event" alone is too common
// a word (e.g. "in the event that") to safely trigger a Notion fetch on.
const MARKETING_EVENTS_PATTERNS = [
  /\bmarketing\s+(events?|calendar)\b/i,
  /\bfield\s+marketing\b/i,
  /\btrade\s*shows?\b/i,
  /\bconferences?\b(?!\s*call)/i,
  /\bwebinars?\b/i,
  /\bupcoming\s+(events?|conferences?|trade\s*shows?|dinners?|webinars?)\b/i,
  /\b(events?|conferences?)\s+(coming\s+up|this\s+(week|month|quarter))\b/i,
  /\bwhat\s+(events?|conferences?)\s+(are|is)\s+(there|upcoming|coming)\b/i,
  /\bare\s+there\s+any\s+(events?|conferences?)\b/i,
];

export function wantsMarketingEvents(text) {
  if (!text) return false;
  return MARKETING_EVENTS_PATTERNS.some((re) => re.test(text));
}
