...
//
// WHY THIS EXISTS
// prompts/system.js concatenates ten-odd context blocks unconditionally:
// sender identity, the sender's profile, their last 8 messages, facts about
// every teammate named in the message, thread context (up to 4000 chars from
// lib/thread-context.js), Notion context (up to 2300 chars from
// lib/context.js), marketing events (up to 2500 chars), calendar, capability
// summary, intent rules, and ~4KB of fixed persona/customer-story text. Every
// one of those has its own local cap and none of them know about each other,
// so the only thing bounding the total is the sum of the individual caps -
// roughly 4k tokens of context on top of a persona block that is already
// ~1.2k tokens.
//
...
//                     it IS the answer.
//  notion_context   - fetched only on a work signal.
//  marketing_events - fetched only on an explicit events question.
