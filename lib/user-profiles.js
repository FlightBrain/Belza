...
export async function updateUserProfile(userId, {
  ...
  // Detect personality/meanness signals - these only make sense for
  // messages actually directed at the bot (intent is set). Ambient channel
  // chatter (intent: null) is teammates talking to each other, not to the
  // bot, so "that was so cringe" about an unrelated topic must not get
  // filed as someone being mean to the bot.
  if (intent) {
...
export function profileToPromptContext(profile, history) {
  ...
  if (history && history.length > 0) {
    ...
    for (const h of recent) {
      const date = h.timestamp ? new Date(h.timestamp).toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: BOT_TZ }) : '';
...
// profile context. Matches on the full display name OR just the first
// name/token, since people almost always refer to teammates by first name
// only ("what does alec do") even when the stored profile has a full name
// ("Alec Sloan").
export function findMentionedTeammates(text, knownUsers, excludeUserId) {
  ...
  for (const u of knownUsers) {
    ...
    const candidates = new Set([u.displayName, firstName]);
    const isMentioned = [...candidates].some((name) => {
      const escaped = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      return new RegExp(`\\b${escaped}\\b`, 'i').test(text);
    });
    if (isMentioned) matches.push(u);
  }
  return matches;
}

// One-off/rerunnable maintenance: rebuild the known-users index from every
// profile already in KV. Needed once because the index was added after
// profiles already existed (a manual channel-history backfill), so those
// pre-existing profiles were never indexed. Safe to rerun any time.
export async function rebuildKnownUsersIndex() {
  ...
  for (const key of keys) {
    ...
    if (profile?.userId && profile?.displayName) {
      known.push({ userId: profile.userId, displayName: profile.displayName });
    }
  }
  await kv.set(KNOWN_USERS_KEY, known);
  return known;
}
