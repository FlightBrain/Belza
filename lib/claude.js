...
const MAX_RETRIES = 3;
// Above this we'd blow Vercel's 60s function budget waiting, so give up and
// let the caller degrade gracefully instead.
...
async function postWithRetry(body) {
  ...
  for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (wait > MAX_WAIT_MS) {
      console.warn(`groq: 429 wants ${wait}ms, over budget, giving up`);
      return last;
    }
    console.warn(`groq: 429, waiting ${wait}ms (attempt ${attempt + 1}/${MAX_RETRIES})`);
