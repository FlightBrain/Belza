...
// A roster we know is incomplete (users.info failed for someone) gets a much
// shorter life so it retries soon instead of being trusted for 6 hours.
const PARTIAL_TTL_MS = 5 * 60 * 1000; // 5 minutes
...
export function isStale(roster, ttlMs = DEFAULT_TTL_MS) {
  ...
  const budget = roster.partial ? Math.min(ttlMs, PARTIAL_TTL_MS) : ttlMs;
  return Date.now() - new Date(roster.fetchedAt).getTime() > budget;
}
...
export async function refreshRoster(channelId) {
  ...
  const { users, failedIds } = await fetchUsers(memberIds);
  ...
  // Anyone users.info failed on keeps their PREVIOUS record rather than
  // vanishing. Dropping them silently made a known teammate unresolvable -
  // their <@U…> tag rendered as "@someone" and the bot claimed not to know
  // someone it has notes on - and permanently destroyed their name history,
  // since the next refresh would find no previous record to carry forward.
  const carriedForward = [];
  for (const id of failedIds) {
    const stale = previousById.get(id);
    if (stale) {
      people.push(stale);
  ...
  const roster = {
    ...
    people,
    // A partial roster must not be trusted for a full TTL. isStale() gives it
    // a much shorter one so the next lookup retries instead of serving a
    // known-incomplete roster for 6 hours.
    partial,
    ...(partial && { failedIds }),
  };
  ...
  console.log(
    `roster: refreshed ${channelId} - ${people.length}/${memberIds.length} members resolved ` +
      `(${people.filter((p) => !p.isBot && !p.deleted).length} active humans)` +
      (partial
        ? `, PARTIAL: ${failedIds.length} failed (${carriedForward.length} served from cache)`
        : '') +
...
export async function invalidateRoster(channelId) {
  if (!channelId) return;
  // Clear the in-memory copy FIRST and unconditionally. When this was after
  // the kv.del, a throwing kv.del skipped it and left the stale roster live on
  // that instance - the invalidation silently did nothing.
...
// Returns { users, failedIds }. The failures MUST be reported, not swallowed:
// a partially-fetched roster that gets saved as complete makes those people
// unresolvable, and refreshRoster needs to know so it can carry their previous
// records forward and shorten the TTL.
async function fetchUsers(userIds) {
  ...
  for (let i = 0; i < userIds.length; i += USER_INFO_CONCURRENCY) {
    ...
    for (const r of results) {
      if (r.__failed) failedIds.push(r.__failed);
      else users.push(r);
    }
  }
...
export function buildPerson(user, previous) {
  ...
  const person = {
    ...
    handle: user.name || '',
    title: profile.title || '',
    // Slack exposes profile.pronouns when someone sets it. Without carrying it
    // through, identityToPromptContext has nothing to emit and the model fills
    // the gap by guessing from the first name - observed live, it called both
    // Kensington and Sacha "she" with no pronoun data anywhere.
